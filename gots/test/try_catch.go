...
func mayFail(shouldFail bool) int {
	if shouldFail {
...
func main() {
	func() {
		defer func() {
			if r := recover(); r != nil {
				e := r
				_ = e // prevent unused variable error
				fmt.Println("Caught an error!")
			}
		}()
		var result int = mayFail(false)
		fmt.Println(result)
	}()
	func() {
		defer func() {
			if r := recover(); r != nil {
				e := r
				_ = e // prevent unused variable error
				fmt.Println("Caught error in test 2")
			}
		}()
		var result int = mayFail(true)
		fmt.Println(result)
	}()
	func() {
		defer func() {
			if r := recover(); r != nil {
				e := r
				_ = e // prevent unused variable error
				fmt.Println("Caught custom error")
			}
		}()
		panic("Custom error")
	}()
