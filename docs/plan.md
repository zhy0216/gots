...
- `super()` handling: generate parent constructor call and initialize embedded fields; clarify embedding strategy.
...
- Unify `typed.Builder` and `types.Checker` (single source of truth) or port missing checks (break/continue, switch case compatibility, builtin arity).
...
- Add tests for optional chaining, nullish coalescing, super, const reassignment, for-of strings, and error cases.
